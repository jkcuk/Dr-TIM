...
/**
 * SurfaceProperty that turns a sphere into a Maxwell fisheye lens.
 * 
 * @see optics.raytrace.core.SceneObjectPrimitive#getNormalisedOutwardsSurfaceNormal(Vector3D p)
 * @author Johannes Courtial
 */
public class MaxwellFisheyeLensSurface extends SurfacePropertyPrimitive
{
	private static final long serialVersionUID = -3077250816001388291L;

	/**
	 * A Maxwell fisheye lens can be embedded into a medium with any refractive index, n.
	 ...
	 * The parameter ratioNSurfaceNSurrounding = n_edge / n_outside enables us to simulate a fisheye lens that has been designed
	 * to be embedded in a material with refractive index n_edge but that is embedded in a material with refractive index
	 * n_outside.
	 * (If ratioNSurfaceNSurrounding = 1, the fisheye lens should work as a fisheye lens normally does.) 
	 */
	private double ratioNSurfaceNSurrounding;	// refractive-index ratio
		
	/**
	 * A fisheye lens can be combined with an invisible sphere to act as a fisheye lens for light rays that hit it
	...
	/**
	 * Creates a new fisheye-lens surface.
	 ...
	 */
	public MaxwellFisheyeLensSurface(double ratioNSurfaceNSurrounding, double transparentTunnelRadius, double transmissionCoefficient, boolean shadowThrowing)
	{
	...
	/**
	 * Creates a new, perfect, fisheye-lens surface.
	 */
	...
	/**
	 * Clone the original fisheye-lens surface
	 * @param original
	 */
	public MaxwellFisheyeLensSurface(MaxwellFisheyeLensSurface original)
	{
		this(
	 ...
	 */
	@Override
	public MaxwellFisheyeLensSurface clone()
	{
		return new MaxwellFisheyeLensSurface(this);
	}
	...
	throws RayTraceException
	{
		...
		{
			throw new RayTraceException("Object with Maxwell-fisheye-lens surface is not of a sphere!");
		}
		...
		try
		{
			...
			// ... and its part that is parallel to R
			...
			{
				// calculate the new starting point
				newStartingPoint = Vector3D.sum(s.getCentre(), R);
			
				// the component of dInside in the direction of R
				Vector3D dInsideR = dInside.getPartParallelTo(R);
				
				// Calculate the new light-ray direction before refraction at the edge of the lens...
				dPrimeInside = Vector3D.difference(dInsideR.getProductWith(2), dInside);
			}
