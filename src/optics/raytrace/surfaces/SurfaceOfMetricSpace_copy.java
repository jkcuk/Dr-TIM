 ...
 */
public abstract class SurfaceOfMetricSpace_copy extends SurfacePropertyPrimitive
implements Derivatives
{
	 ...
	 */
	public SurfaceOfMetricSpace_copy(SceneObject surface, double deltaTau, double deltaXMax, int maxSteps, IntegrationType integrationType, double transmissionCoefficient, boolean shadowThrowing)
	{
	 ...
	 * @param original
	 */
	public SurfaceOfMetricSpace_copy(SurfaceOfMetricSpace_copy original)
	{
		this(
				original.getSurface(),
	 ...
	 */
	@Override
	public abstract SurfaceOfMetricSpace_copy clone();
	 ...
	 */
	public abstract Matrix getEpsilonMuTensor(Vector3D x);
	 ...
	 */
	public Vector3D correctLengthOfK(Vector3D x, Vector3D k)
	{
		return k;
	}
...
//			if(r.isRayWithTrajectory())
//			{
//				// This method was called as the light ray, assuming that it travelled in a straight line,
//				// intersected the surface (boundary) of the metric volume.
//				// When the intersection with the surface was found, the position of the intersection was added to the list of the ray's intersection points.
//				// But the intersection was in an outwards direction, which means that the ray originated inside the volume.
//				// This means that it (normally) does not propagate along a straight line, but along a geodesic defined by the metric.
//				// (And finding this geodesic is what this class is, of course, all about.)
//				// Before calculating that geodesic, we therefore need to remove the intersection with the surface from the ray's list of intersection points.
//				((RayWithTrajectory)r).removeLastIntersectionPoint();
//			}
//			
//			// set the ray's wave vector according to the ray direction
//			r.setK(calculateK(i.p, r.getD()));
//
//			// the ray must have originated inside the volume, so trace it from here
//			return getColourUponStartingWithinVolume(r, scene, l, maxSteps, traceLevel, raytraceExceptionHandler);
//		}
//		else
//		{
//			// the ray started from outside the volume
//			
//			// System.out.println("Inwards, position = "+i.p+", traceLevel = "+traceLevel);
//
//			// create a new ray, starting from the intersection point with the surface and with the right k vector...
//			Ray rI = getInwardsRefractedRay(r, i);
//			
//			// ... and trace it through the volume
//			return getColourUponStartingWithinVolume(rI, scene, l, maxSteps, traceLevel, raytraceExceptionHandler);
//		}
	}
	...
	throws EvanescentException
	{
		...
		Matrix nTensor = getEpsilonMuTensor(position);
	...
	throws RayTraceException
	{
		...
		for(; stepsLeft > 0; stepsLeft--)
		{
			...
			// translate the iteration step into dx and dk
			Vector3D dX = new Vector3D(dfdt[0], dfdt[1], dfdt[2]);
			Vector3D dK = new Vector3D(dfdt[3], dfdt[4], dfdt[5]);
			...
			// TODO
			// is the step is longer than the maximum step length?
			double deltaX = dX.getLength();
			if(deltaX > deltaXMax)
			{
				// yes; take a smaller step in the same direction
				// TODO improve this?
				switch(integrationType)
				{
				case EULER:
					// the current step is larger than the max step size; adjust it
					dX = dX.getProductWith(deltaXMax / deltaX);
					dK = dK.getProductWith(deltaXMax / deltaX);
					break;
				case RK4:
				default:
					// Runge-Kutta method
					RungeKutta.calculateDeltaF(
							0,	// t
							deltaTau*deltaXMax / deltaX,	// dt
							f,
							dfdt,
							this	// model
							);
					dX = new Vector3D(dfdt[0], dfdt[1], dfdt[2]);
					dK = new Vector3D(dfdt[3], dfdt[4], dfdt[5]);
				}
			}
...
//			// calculate dx / dtau and dk / dtau
//			Vector3D dXdTau = dXdTau(r.getP(), r.getK());
//			Vector3D dKdTau = dKdTau(r.getP(), r.getK());
//			
//			// System.out.println("SurfaceOfMetricSpace::getColourUponStartingWithinVolume: dXdTau="+dXdTau+", dKdTau="+dKdTau);
//
//			
//			
//			Vector3D dX = dXdTau.getProductWith(tau);
//			Vector3D dK = dKdTau.getProductWith(tau);
			...
			if(!surface.insideObject(newP))
			{
				...
				double factor = Vector3D.difference(i.p, r.getP()).getLength() / dX.getLength();
				// and estimate the corresponding value of k
				r.setK(Vector3D.sum(r.getK(), dK.getProductWith(factor)));
				
				// TODO set new position, direction, and k vector 
				
				here
				System.out.println("SurfaceOfMetricSpace::getColourUponStartingWithinVolume: hitting surface of volume; H = "+calculateHamiltonian(r.getK(), getEpsilonMuTensor(i.p))+" (should be 0)");
