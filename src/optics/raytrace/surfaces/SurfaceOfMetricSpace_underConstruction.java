 ...
 */
public abstract class SurfaceOfMetricSpace_underConstruction extends SurfacePropertyPrimitive
implements FirstOrderDifferentialEquations, EventHandler
{
	private static final long serialVersionUID = -4836545456734020700L;
	 ...
	 */
	public SurfaceOfMetricSpace_underConstruction(SceneObject surface, double deltaTau, double deltaXMax, int maxSteps, IntegrationType integrationType, double transmissionCoefficient, boolean shadowThrowing)
	{
	 ...
	 * @param original
	 */
	public SurfaceOfMetricSpace_underConstruction(SurfaceOfMetricSpace_underConstruction original)
	{
		this(
				original.getSurface(),
	 ...
	 */
	@Override
	public abstract SurfaceOfMetricSpace_underConstruction clone();
...
//	}
	...
	{
		// the first three values of y are the position, ...
		Vector3D p = new Vector3D(y[0], y[1], y[2]);
		
		// ... the last three are the momentum
		Vector3D k = new Vector3D(y[3], y[4], y[5]);
		
		// calculate dp / dtau...
		Vector3D dpdtau = dXdTau(p, k);
		
		// ... and dk / dtau, ...
		Vector3D dkdtau = dKdTau(p, k);
		
		// ... and construct from these d(p,k) / dt
		yDot[0] = dpdtau.x;
		yDot[1] = dpdtau.y;
		yDot[2] = dpdtau.z;
		yDot[3] = dkdtau.x;
...
//			}
	...
	private KAndX calculateDeltaKAndDeltaX(Ray r, double deltaTau)
	{
		...
		// see http://commons.apache.org/proper/commons-math/userguide/ode.html
		FirstOrderIntegrator dp853 = new DormandPrince853Integrator(deltaTau/100, 100*deltaTau, 1.0e-10, 1.0e-10);
		
		dp853.integrate(this, 0.0, y, 0.1, y); // now y contains final state at time t=0.1
...
//		}
		...
		// return the result
		return new KAndX(
				new Vector3D(dydt[3], dydt[4], dydt[5]),	// dk
				new Vector3D(dydt[0], dydt[1], dydt[2])	// dx
				);
	}
		...
		for(; stepsLeft > 0; stepsLeft--)
		{
			...
			if(!surface.insideObject(newP))
			{
				...
				try
				{
					...
					return scene.getColourAvoidingGivenIntersections(
...
//			r.setK(Vector3D.sum(r.getK(), dKdTau(r.getP(), r.getK()).getProductWith(tau)));
	...
	/* (non-Javadoc)
	 * @see org.apache.commons.math3.ode.events.EventHandler#g(double, double[])
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see org.apache.commons.math3.ode.events.EventHandler#eventOccurred(double, double[], boolean)
	 */
	@Override
