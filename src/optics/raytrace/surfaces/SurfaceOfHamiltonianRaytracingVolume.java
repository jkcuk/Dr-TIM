 ...
 * the inside of the surface --- to be a volume of space in which rays don't travel in straight lines but
 * according to Hamilton's equations (Eqns (41) in [1]).
 * 
 ...
 * and calculate the wave vector corresponding to any direction at any given position.
 * 
 * References:
 ...
 */
public abstract class SurfaceOfHamiltonianRaytracingVolume extends SurfacePropertyPrimitive
implements Derivatives
{
	 ...
	 */
	public SurfaceOfHamiltonianRaytracingVolume(SceneObject surface, double deltaTau, double deltaXMax, int maxSteps, IntegrationType integrationType, double transmissionCoefficient, boolean shadowThrowing)
	{
	 ...
	 * @param original
	 */
	public SurfaceOfHamiltonianRaytracingVolume(SurfaceOfHamiltonianRaytracingVolume original)
	{
		this(
				original.getSurface(),
	 ...
	 */
	@Override
	public abstract SurfaceOfHamiltonianRaytracingVolume clone();
	
	
	//
	// methods describing Hamilton equations;
	...
	// 
	// d x_i / d \tau =  \partial H / \partial k_i, (1)
	// d k_i / d \tau = -\partial H / \partial x_i, (2)
	// 
	// where i=1, 2, 3, k = (k_1, k_2, k_3) is the wave vector at position x = (x_1, x_2, x_3), and the Hamiltonian is (Eqn (40) in [1] with f(x) = 1)
	// 
	// H = (k.n.k - det(n)),
	...
	// Mathematica can derive formulas that allow calculation of the RHS of Eqns (1) and (2) for any given metric tensor;
	// these then need to be returned by the methods dXdTau and dKdTau.
