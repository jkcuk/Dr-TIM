...
public class EditableLensTOTetrahedron extends EditableSceneObjectCollection implements ActionListener
{
	...
	protected LabelledDoublePanel baseRadiusLine, heightLine, aLine, aPrimeLine, bLine, bPrimeLine, interfaceTransmissionCoefficientLine, frameRadiusLine;
	...
	protected void calculateVertices()
	{
		...
		double upperHP, upperHE, lowerHP, lowerHE;
		
		// check which vertex is the upper vertex
		if(aPrime > bPrime)
		{
			// vertex 1 is the upper vertex
			upperHE = a*height;
			upperHP = aPrime*height;
			lowerHE = b*height;
			lowerHP = bPrime*height;
		}
		else
		{
			// vertex 2 is the upper vertex
			upperHE = b*height;
			upperHP = bPrime*height;
			lowerHE = a*height;
			lowerHP = aPrime*height;
		}
		
		// upper inner vertex is a distance upperHE above the base centre in EM space
		upperInnerVertexE = Vector3D.sum(baseCentre, topDirection.getProductWith(upperHE));

		// upper inner vertex is a distance upperHP above the base centre in physical space
		upperInnerVertexP = Vector3D.sum(baseCentre, topDirection.getProductWith(upperHP));

		// lower inner vertex is a distance lowerHE above the base centre in EM space
		lowerInnerVertexE = Vector3D.sum(baseCentre, topDirection.getProductWith(lowerHE));

		// lower inner vertex is a distance lowerHP above the base centre in physical space
		lowerInnerVertexP = Vector3D.sum(baseCentre, topDirection.getProductWith(lowerHP));
	}
