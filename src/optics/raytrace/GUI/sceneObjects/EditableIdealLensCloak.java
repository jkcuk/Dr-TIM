 ...
 * @author Johannes
 */
public class EditableIdealLensCloak extends EditableSceneObjectCollection implements ActionListener
{
	 ...
	 * @param studio
	 */
	public EditableIdealLensCloak(SceneObject parent, Studio studio)
	{
		this(
				"Ideal-lens cloak",
	 ...
	 * @param original
	 */
	public EditableIdealLensCloak(EditableIdealLensCloak original)
	{
		this(
	...
	@Override
	public EditableIdealLensCloak clone()
	{
		return new EditableIdealLensCloak(this);
	}
	 ...
	 */
	public static EditableIdealLensCloak getEditableIdealLensCloakUsingBaseLensFocalLength(
			String description,
			Vector3D baseCentre,
			Vector3D frontDirection,
			Vector3D rightDirection,
		...
		)
	{
		...
		// take the inverse:
		//   h1E = f h1P / (f - h1P)
		return new EditableIdealLensCloak(
				description,
				baseCentre,
				frontDirection,
				rightDirection,
...
//		System.out.println("EditableLensTO::addWindows: f00="+f00);
		...
		double focalLengthUpperInner = (f00*(h1 - h2)*(h2 - h3)*R)/(h1*h3*Math.sqrt(4*h2*h2 + R*R));
		// System.out.println("EditableLensTO::addWindows: focalLengthUpperInner="+focalLengthUpperInner);
		
		double focalLengthLowerInner = -(((h1 - h2)*(f00*(h1 - h3) + h1*h3)*R)/(h2*h3*Math.sqrt(4*h1*h1 + R*R)));
		// System.out.println("EditableLensTO::addWindows: focalLengthLowerInner="+focalLengthLowerInner);
		
		double focalLengthUpperInnerVertical = ((h2 - h3)*(f00*(h1 - h3) + h1*h3)*R)/(2*Math.sqrt(3)*h1*h2*h3);
		// System.out.println("EditableLensTO::addWindows: focalLengthUpperInnerVertical="+focalLengthUpperInnerVertical);
		
		double focalLengthLowerInnerVertical = -(((h1 - h2)*(f00*(h1 - h3) + h1*h3)*R)/(2*Math.sqrt(3)*h1*h2*h3));
		...
		)
	{
		...
		case PHASE_HOLOGRAM_OF_LENS:
			...
			// as follows:
			Vector3D outwardsNormal = Vector3D.crossProduct(v12, v13).getNormalised();
...
//		basicParametersPanel.add(showPlaceholderSurfacesCheckBox, "wrap");
	 ...
	 */
	@Override
	public EditableIdealLensCloak acceptValuesInEditPanel()
	{
