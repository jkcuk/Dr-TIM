...
/**
 * A resonator formed by a pair of Eaton lenses
 * 
...
//				));
		...
		// tetrahedron
		double eatonLensRadius = 1;
		double ratioNSurfaceNSurrounding = 1;
		double transparentTunnelRadius = 0;
		boolean shadowThrowing = true;
		double transmissionCoefficient = 0.98;
		double centre2vertexFactor = 1;	// 1 for Eaton lenses touching

		Vector3D centre = new Vector3D(1, 1/Math.sqrt(3),1/Math.sqrt(6));
		
		scene.addSceneObject(
				new EatonLens(
						"Eaton lens 1",	
						Vector3D.difference(new Vector3D(0, 0, 0), centre).getProductWith(centre2vertexFactor),	// centre
						eatonLensRadius,	// radius
		...
		scene.addSceneObject(
				new EatonLens(
						"Eaton lens 2",	
						Vector3D.difference(new Vector3D(1, 1/Math.sqrt(3), 0), centre).getProductWith(centre2vertexFactor),	// centre
						eatonLensRadius,	// radius
						ratioNSurfaceNSurrounding,
						transparentTunnelRadius,
						transmissionCoefficient,	// transmission coefficient
		...
		scene.addSceneObject(
				new EatonLens(
						"Eaton lens 3",	
						Vector3D.difference(new Vector3D(2, 0, 0), centre).getProductWith(centre2vertexFactor),	// centre
						eatonLensRadius,	// radius
						ratioNSurfaceNSurrounding,
						transparentTunnelRadius,
						transmissionCoefficient,	// transmission coefficient
		...
		scene.addSceneObject(
				new EatonLens(
						"Eaton lens 4",	
						Vector3D.difference(new Vector3D(1, 1/Math.sqrt(3), Math.sqrt(8/3)), centre).getProductWith(centre2vertexFactor),	// centre
						eatonLensRadius,	// radius
						ratioNSurfaceNSurrounding,
						transparentTunnelRadius,
						transmissionCoefficient,	// transmission coefficient
