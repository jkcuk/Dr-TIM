 ...
 */
public class ThreeSkewLensRotationVisualiser_old extends NonInteractiveTIMEngine
{
...
//	protected double telescopeMagnification;
	 ...
	 */
	public ThreeSkewLensRotationVisualiser_old()
	{
...
//				+ (showLens3?"":" (lens 3 not shown)")
				...
				+ (!topView?" aperture size "+cameraApertureSize+" view centre "+cameraViewCentre:"")
				+ ((topView && showTrajectories)?" (with trajectories)":"")
				+ ((topView && showEyeAndEyeTrajectories)?" (with eye trajectories)":"")
				+ (topView?" (top view)":"")
				+ (rotateLattice?"lattice rotated":"")
				;
	}
...
//				System.out.println("ThreeSkewLensRotation::populateStudio: intersection direction of lens planes: "+lensPlaneIntersectionDirection);
	...
	private JCheckBox topViewCheckBox, showTrajectoriesCheckBox, showEyeAndEyeTrajectoriesCheckBox;	//, rotateLatticeCheckBox;//showLens3CheckBox, 
...
//		latticePanel.add(zVectorPanel, "span"); 
		...
		cameraPanel.add(new LabelledComponent("Camera aperture", cameraApertureSizeComboBox), "span");
		...
		cameraPanel.add(showTrajectoriesCheckBox, "span");
...
//		}
	...
	{
		(new ThreeSkewLensRotationVisualiser_old()).run();
	}
}
