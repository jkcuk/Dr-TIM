...
public class PolyhedralShiftyCloakJanusWormholeVisualiser extends NonInteractiveTIMEngine
{
	...
	private double n; //the multiple of p4-p1 by which the wormhole should shift the image.
	private double l0;// the separation between lens l1 and l4 in the wormhole. this will determine the shift amount.
	private boolean setF2;
	...
	public PolyhedralShiftyCloakJanusWormholeVisualiser()
	{
	...
	public void writeParameters(PrintStream printStream)
	{
		...
		case WORMHOLE:
			...
			printStream.println("l0 ="+l0);
			printStream.println("setF2 ="+setF2);
			break;
		case NONE:
			break;
		}
...
//				));
		...
		case WORMHOLE:
			...
			if(setF2) {
				f2 = -((2*f1*f1+Math.sqrt(f1*f1*l0*(l0+2*f1*n-l0*n)))/(2*f1+l0-l0*n));
			}
			
			
			// this will now be done by a shifty cloak... double d = (2*(f1+f2)*(f2/f1)); //separation along z axis of virtual lenses 2 and 3
			
			Vector3D l3Pos = Vector3D.sum(centre, normal.getProductWith(MyMath.TINY + (outsideCubeSideLength2-insideCubeSideLength2)));
			Vector3D l2Pos = Vector3D.sum(centre, normal.getProductWith(-MyMath.TINY - (outsideCubeSideLength2-insideCubeSideLength2)));
			
			//Vector3D lastLens = Vector3D.sum(objectCentre[0], normal.getProductWith(2*f1+2*f2+d));
			if(Vector3D.getDistance(centre, Vector3D.sum(l3Pos, normal.getProductWith(f1+f2)))>= outsideCubeSideLength2) {
				System.err.println("Inside lenses outside of inner cube!");
				break;
			}
			lens1 = new Disc(
					"first lens",// description,
					Vector3D.sum(l2Pos, normal.getProductWith(-f1-f2)),// centre,
					normal,
			...
			lens3 = new Disc(
					...
					l3Pos,
					//Vector3D.sum(objectCentre[0], normal.getProductWith(MyMath.TINY + (insideCubeSideLength1-insideCubeSideLength2)/2)),// centre,
					normal,
					insideCubeSideLength2/2.01,
					new IdealThinLensSurfaceSimple(
							l3Pos,
			...
			lens4 = new Disc(
					...
					Vector3D.sum(l3Pos, normal.getProductWith(f1+f2)),
					//lastLens,// centre,
					normal,
					insideCubeSideLength2/2.01,
					new IdealThinLensSurfaceSimple(
			...
			System.out.println("first centre"+Vector3D.sum(l2Pos, normal.getProductWith(-f1-f2))+", second centre "+ l2Pos);
			System.out.println("third centre"+l3Pos+", last centre "+ Vector3D.sum(l3Pos, normal.getProductWith(f1+f2)));
		...
		case JANUS:
			...
			break;
		case WORMHOLE:
			//d is always given by the following
			//double d = (2*(f1+f2)*(f2/f1));
			double d = (2*f2*(f1+f2)*(f1+f2-n*f2))/ (f1*f1 + f2*f2*(-1+n) )	;
			//now take into account the existing separation between l2 and l3
			if(setF2) {
				d = (l0*(l0+2*f1*n-10*n)+2*Math.sqrt(f1*f1*l0*(l0+2*f1*n-l0*n)))/(2*f1+l0-l0*n);
			}
			double shiftNeeded = d - (2*MyMath.TINY + 2*(outsideCubeSideLength2-insideCubeSideLength2));
			
			delta2 = delta1.getNormalised().getProductWith(-shiftNeeded);
			System.out.println("shift= "+(n*(d+2*(f1+f2))));
			cloakCenter1 = centre;
			//the inner cloak must now also be shifted exclusively over two of the lenses...
			cloakCenter2 = Vector3D.sum(centre, delta1.getNormalised().getProductWith(-insideCubeSideLength2/2));
			System.out.println(cloakCenter2);
			hole = true;
			break;
		case NONE:
			break;
	...
	private LabelledDoublePanel f1Panel, f2Panel, lensTransmissionPanel, nPanel, l0Panel;// the separation between lens l1 and l4 in the wormhole. this will determine the shift amount.
	private JCheckBox setF2CheckBox;
	...
	protected void createInteractiveControlPanel()
	{
		...
		nPanel = new LabelledDoublePanel("n");
		nPanel.setNumber(n);
		lensPanel.add(nPanel, "wrap");
		
		l0Panel = new LabelledDoublePanel("l0");
		l0Panel.setNumber(l0);
		lensPanel.add(l0Panel, "wrap");
		
		setF2CheckBox = new JCheckBox("set f2");
		setF2CheckBox.setSelected(setF2);
		lensPanel.add(setF2CheckBox, "wrap");
	...
	protected void acceptValuesInInteractiveControlPanel()
	{
		...
		setF2 = setF2CheckBox.isSelected();
		l0 = l0Panel.getNumber();
	...
	{
		(new PolyhedralShiftyCloakJanusWormholeVisualiser()).run();
	}
}
