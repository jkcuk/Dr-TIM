 ...
 */
public class LensTOTetrahedronMovie
{
	public static final boolean SHOW_TETRAHEDRON = false;
	...
	public static final double FOCUS_Z = 9.8;
	...
	public static String getFilename(int frame)
	{
		return "LensTOTetrahedron"
				...
				+(SHOW_TIM?" (with Tim)":"")
				+(SPHERE_ARRAY?" (with sphere array, z="+FOCUS_Z+")":"")
				+(CYLINDER_ARRAY?" (with cylinder array, z="+FOCUS_Z+")":"")
				+(TEST?" (test)":"")
		...
		if(SPHERE_ARRAY)
		{
			...
			scene.addSceneObject(
					new EditableArray(
							...
							FOCUS_Z, FOCUS_Z, 1,	// 9, 11.5, 0.2,	// zMin, zMax, dz
							arrayUnitCell,
							scene, 
							studio
					)
		...
		if(CYLINDER_ARRAY)
		{
			...
			for(double y=yMin; y<=yMax; y+=delta)
					scene.addSceneObject(new Cylinder(
									...
									new Vector3D(xMin, y, FOCUS_Z),	// startPoint
									new Vector3D(xMax, y, FOCUS_Z),	// endPoint
									0.01,	// radius
			...
			for(double x=xMin; x<=xMax; x+=delta)
				scene.addSceneObject(new Cylinder(
								...
								new Vector3D(x, yMin, FOCUS_Z),	// startPoint
								new Vector3D(x, yMax, FOCUS_Z),	// endPoint
								0.01,	// radius
								SurfaceColour.WHITE_SHINY,	// surfaceProperty
								scene,	// parent
								studio
...
//		));
		...
		double angle = 2*Math.PI*frame/getNoOfFrames();
		...
		EditableRelativisticAnyFocusSurfaceCamera camera = new EditableRelativisticAnyFocusSurfaceCamera(
				...
				new EditableParametrisedPlane(
						"focussing plane",
						new Vector3D(0, 0, FOCUS_Z),	// point on plane
