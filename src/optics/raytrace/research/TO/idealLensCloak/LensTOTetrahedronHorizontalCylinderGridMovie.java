 ...
 */
public class LensTOTetrahedronHorizontalCylinderGridMovie
{
	...
	{
		return "LensTOTetrahedron physical-space planes with"
	...
	public static Studio createStudio(int frame)
	{
		...
		double arrayZ = 10 - 2.*1./3. + 3.*2./3.*frame/(getNoOfFrames()-1);
		...
		if(SPHERE_ARRAY)
		{
			...
			scene.addSceneObject(
					new EditableArray(
							...
							arrayZ, arrayZ, 1,	// 9, 11.5, 0.2,	// zMin, zMax, dz
							arrayUnitCell,
							scene, 
							studio
					)
		...
		if(CYLINDER_ARRAY)
		{
			...
			for(double y=yMin; y<=yMax; y+=delta)
					scene.addSceneObject(new Cylinder(
									...
									new Vector3D(xMin, y, arrayZ),	// startPoint
									new Vector3D(xMax, y, arrayZ),	// endPoint
									0.01,	// radius
			...
			for(double x=xMin; x<=xMax; x+=delta)
				scene.addSceneObject(new Cylinder(
								...
								new Vector3D(x, yMin, arrayZ),	// startPoint
								new Vector3D(x, yMax, arrayZ),	// endPoint
								0.01,	// radius
								shadowlessWhite,	// surfaceProperty
								scene,	// parent
								studio
...
//		));
		...
		EditableRelativisticAnyFocusSurfaceCamera camera = new EditableRelativisticAnyFocusSurfaceCamera(
				...
				new EditableParametrisedPlane(
						"focussing plane",
						new Vector3D(0, 0, arrayZ),	// point on plane
